// headless ui componentes dinamicos de tailwindcss
...
type DevTreeInputProps = {
  item: DevTreeLinkSocial;
  handleUrlChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  handleEnableLink: (name: DevTreeLinkSocial["name"]) => void;
};

export function DevTreeInput({
  item,
  handleUrlChange,
  handleEnableLink,
}: DevTreeInputProps) {
  return (
    <>
      <div className=" bg-white shadow-sm p-5 flex items-center gap-3">
        ...
        <input
          type="text"
          className=" flex-1 border bg-gray-100"
          name={item.name}
          value={item.url}
